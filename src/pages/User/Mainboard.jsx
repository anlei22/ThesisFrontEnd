...
import { Star, Search, Filter, Dog, Cat, Bird, Fish, Rabbit, ChevronDown, ChevronUp, MapPin, MessageCircle, X, Send, Bot, TrendingUp, ShoppingBag, DollarSign, Award, Users, Home, User } from 'lucide-react';
...
const Mainboard = () => { 
  ...
  const categories = [
    ...
    { id: 'lapulapu', name: 'Lapu-Lapu', icon: Fish },
    { id: 'manok', name: 'Manok', icon: Filter },
    { id: 'palaka', name: 'Palaka', icon: Filter },
    { id: 'pato', name: 'Pato / Itik', icon: Bird },
    { id: 'pusit', name: 'Pusit', icon: Filter },
    { id: 'rabbit', name: 'Rabbit', icon: Rabbit },
    { id: 'sugpo', name: 'Sugpo / Hipon', icon: Fish },
    { id: 'talaba', name: 'Talaba', icon: Filter },
    { id: 'talakitok', name: 'Talakitok', icon: Fish },
    { id: 'tahong', name: 'Tahong', icon: Filter },
  ...
  const filteredUsers = React.useMemo(() => {
    ...
    return allUsers.filter(user => {
      ...
      const nameMatch = user.name.toLowerCase().includes(userSearchTerm.toLowerCase());
      
      // Filter by user type
      const typeMatch = userSearchType === 'both' || user.type === userSearchType;
  ...
  const selectedCategoryData = categories.find(cat => cat.id === postCategory);
  const selectedLocationData = locations.find(loc => loc.id === postLocation);

  // Helper function for rank badges
  const getRankBadge = (index) => {
    if (index === 0) return { color: 'bg-yellow-400 text-yellow-900', border: 'border-yellow-400' };
    if (index === 1) return { color: 'bg-gray-400 text-gray-900', border: 'border-gray-400' };
    if (index === 2) return { color: 'bg-orange-400 text-orange-900', border: 'border-orange-400' };
    return { color: '', border: 'border-transparent' };
  };

  // Get sellers and buyers for top participants
  const sellers = allUsers.filter(user => user.type === 'seller').slice(0, 10);
  ...
  // Mobile Search Interface - Updated
  const MobileSearchInterface = () => (
    <div className="space-y-4">
      ...
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm`}>
        ...
        <div className="p-4">
          {/* User Type Toggle */}
          <div className={`flex p-1 rounded-lg mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            <button
              onClick={() => setUserSearchType('both')}
              className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                userSearchType === 'both'
                  ? darkMode ? 'bg-green-600 text-white' : 'bg-white text-green-600 shadow-sm'
            ...
            <button
              onClick={() => setUserSearchType('seller')}
              className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                userSearchType === 'seller'
                  ? darkMode ? 'bg-green-600 text-white' : 'bg-white text-green-600 shadow-sm'
                  : darkMode ? 'text-gray-400' : 'text-gray-500'
              }`}
            >
            ...
            <button
              onClick={() => setUserSearchType('buyer')}
              className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                userSearchType === 'buyer'
                  ? darkMode ? 'bg-green-600 text-white' : 'bg-white text-green-600 shadow-sm'
                  : darkMode ? 'text-gray-400' : 'text-gray-500'
              }`}
            >
        ...
        <div className="w-full">
          {activeTab === 'home' ? (
            <>
              ...
              <div className={`lg:hidden sticky top-16 z-40 border-b transition-colors ${
              ...
              <div className="hidden lg:flex gap-[20px] mt-6 px-6 lg:px-12">
                {/* LEFT ASIDE - Post Filters & User Search */}
                <aside className="w-72 mt-6 space-y-4 sticky top-6 h-fit">
                  {/* Post Filters Card */}
                ...
                {/* RIGHT ASIDE - Top Participants */}
                <aside className="w-[28rem] mt-6">
                  <div className={`${
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <div className={`p-2 rounded-lg ${darkMode ? 'bg-green-600' : 'bg-green-100'}`}>
                            <TrendingUp className={`w-5 h-5 ${darkMode ? 'text-white' : 'text-green-600'}`} />
                          </div>
                        ...
                        {currentParticipants.map((user, index) => {
                          ...
                          return (
                            <div
                              ...
                              } rounded-xl p-4 border-l-4 ${rankBadge.border} transition-all duration-200 hover:shadow-md group`}
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                  ...
                                  <div className="relative flex-shrink-0">
                                    ...
                                    />
                                    {index < 3 && (
                                      <div className={`absolute -top-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${rankBadge.color} ring-2 ${darkMode ? 'ring-gray-700' : 'ring-white'}`}>
                                        {index + 1}
                                      </div>
                                    )}
                                    {index >= 3 && (
                                      ...
                                      } ring-2 ${darkMode ? 'ring-gray-700' : 'ring-white'}`}>
                                        {index + 1}
                                      </div>
                                    )}
                                  </div>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className={`flex items-center gap-1 text-xs mb-2 ${
                                      ...
                                      <span className="truncate">{userLocation?.name || 'Unknown Location'}</span>
                                    </div>
                                ...
                                {/* Action Button */}
                                <button className={`ml-3 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 flex-shrink-0 ${
                                  darkMode
            ...
            // For Chat & Profile -> Centered
            <div className="max-w-3xl mx-auto px-4 lg:px-0">{renderMainContent()}</div>
          )}
        </div>
