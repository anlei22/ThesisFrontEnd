...
import { 
 ...
 const RegisterModal = ({ isOpen, onClose, onSwitchToLogin, onRegistrationComplete, darkMode }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    birthday: '',
    username: '', // <-- renamed from idNumber (Government ID used as username)
    password: '',
    confirmPassword: '',
    // Step 4 - ID Upload
    idPhoto: null,
  ...
  const handleSubmit = async (e) => {
    ...
    if (registrationStep === 4) {
      ...
      try {
        ...
        // Inform parent (you can set a 'pendingApproval' flag in your app state there)
        ...
        // Show final Thank You / Pending Approval screen (Step 5)
      ...
      // On final screen, buttons handle actions (no submit)
    }
  };
  ...
  const resetForm = () => {
    setFormData({
      ...
      verificationCode: '',
      birthday: '',
      username: '', // reset username (Gov ID)
      password: '',
      confirmPassword: '',
      idPhoto: null,
      selfieWithId: null
  ...
  const stepTitles = [
    ...
    'Personal Details',
    'ID Verification',
    'Registration Complete' // new step 5
  ];
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-lg flex items-center justify-center z-50 p-4">
      <div   className={`relative w-full max-w-lg max-h-[80vh] rounded-2xl shadow-2xl overflow-y-auto transition-all duration-300 ${
    darkMode
  ...
  <div className="flex items-center space-x-2">
    {[1, 2, 3, 4, 5].map((step, index) => (
      <React.Fragment key={step}>
        ...
        {index < 4 && (
          <div
            className={`w-2 sm:w-8 h-1 rounded-full transition-all duration-300 ${
              registrationStep > step
                ? "bg-green-600"
                : darkMode
                ? "bg-gray-700"
        ...
        <div className="px-8 pb-8 max-h-96 overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Step 5: Registration Complete / Awaiting Approval */}
            {registrationStep === 5 && (
              <div className="text-center py-6">
                ...
                <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  Please wait for <span className="font-semibold">admin approval</span>. You’ll be able to log in once approved.
                </p>
                ...
                <div className="flex items-center justify-center gap-3 mt-6">
                  <button
                    type="button"
                    onClick={() => {
                      // optional: keep form data if you want; here we reset
                      resetForm();
...
<div className="text-center">
       ...
       <button
  type="button"
  onClick={onSwitchToLogin} // ✅ this triggers switch
